...
- **Convite** — grupo, código/link, e-mail do convidado, data de criação, prazo de validade, status (válido/expirado/utilizado).
- **Participação** — relação entre usuário e grupo, cota, estado, datas de entrada/saída e confirmação de acesso.
- **Transação** — membro, grupo, valor, vencimento, data de pagamento, comprovante e status, administrador que validadou, data da validação.
...
- Um Grupo tem muitos convites.
- Um Convite pertence a um Grupo.
- Um Convite aceito confirma o vínculo do novo membro com o grupo.
